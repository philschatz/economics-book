...
![Keynesian view of the AD/AS model shows that with a horizontal AS, a decrease in demand leads to a decrease in output, but no decrease in prices.](../resources/CNX_Econ_C25_031.jpg "The Keynesian View of the AD/AS Model uses an SRAS curve, which is horizontal at levels of output below potential and vertical at potential output. Thus, when beginning from potential output, any decrease in AD affects only output, but not prices; any increase in AD affects only prices, not output."){: #CNX_Econ_C25_031 data-media-type="image/jpg" data-title="The Keynesian AD/AS Model"}
...
<span data-type="media" id="ch25mod02_link01m" data-alt="QR Code representing a URL"> ![QR Code representing a URL](../resources/Diane_Rehm.png){: data-media-type="image/png"} </span>
</div>
